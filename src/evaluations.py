...
Output files:
    ...
    Global:     reports/runs/evaluation_summary.csv               (all models × splits)
                reports/runs/REPORT.md                            (human-readable summary)
"""
import json
import logging
...
def write_run_report_md(runs_dir: Path = RUNS_DIR) -> Path | None:
    """Render a human-readable Markdown summary of the latest pipeline run.

    Pulls evaluation_summary.csv, feature_selection_report.csv and known plot
    paths into a single REPORT.md at the root of runs_dir. Renders inline on
    GitHub for public-repo browsing.
    """
    summary_path = runs_dir / "evaluation_summary.csv"
    if not summary_path.exists():
        logger.info("No evaluation_summary.csv — skipping REPORT.md")
        return None
    ...
    lines.append("")
    lines.append("Auto-generated by `make pipeline` (`src/evaluations.py:write_run_report_md`).")
    lines.append("")

    if val_auc_col and not summary.empty:
        best = summary.loc[summary[val_auc_col].idxmax()]
        lines.append("## Best model")
        lines.append("")
        lines.append(f"- **Model**: `{best['model']}`")
        lines.append(f"- **Val AUC**: {best[val_auc_col]:.4f}")
    ...
    combined_roc = runs_dir / "combined_roc.png"
    if combined_roc.exists():
        lines.append("## Combined ROC")
        lines.append("")
        lines.append(f"![Combined ROC]({combined_roc.name})")
        lines.append("")

    figures_dir = runs_dir.parent / "figures"
    fi_pct = figures_dir / "feature_importance_pct.png"
    if fi_pct.exists():
        lines.append("## Feature importance (tree models)")
        lines.append("")
        lines.append(f"![Feature importance](../figures/{fi_pct.name})")
        lines.append("")

    pfi = figures_dir / "permutation_importance.png"
    if pfi.exists():
        lines.append("## Permutation importance")
        lines.append("")
        lines.append(f"![Permutation importance](../figures/{pfi.name})")
        lines.append("")

    fs_path = runs_dir / "feature_selection_report.csv"
    if fs_path.exists():
